...
WORKDIR /src
COPY ["Sklep-internetowy.csproj", "."]
RUN dotnet restore "./Sklep-internetowy.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./Sklep-internetowy.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Sklep-internetowy.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "Sklep-internetowy.dll"]